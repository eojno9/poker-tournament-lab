...
## v2.9 Real HRC Batch Rehearsal

v2.9 is a controlled rehearsal against 28 real repo-external raw HRC zip files. It does not import raw HRC zip files into the product DB, does not connect to product import routes, and does not commit raw zip originals or generated artifact JSON.

### Inventory Preflight

The real raw HRC folder was inspected read-only before the batch rehearsal:

- total zip files: 28
- `settings.json` present: 28
- `nodes/*.json` present: 27
- `READY_FOR_DRY_RUN`: 27
- `INSUFFICIENT_STRUCTURE`: 1
- `UNREADABLE_ZIP`: 0
- raw zip copy/extract/commit: no

### Controlled Batch Dry-run

v2.9 used the existing `hrc:dry-run:batch` command for an explicit controlled dry-run rehearsal.
...
Generated artifact JSON was moved to an external backup folder for review and was not committed to the repository. The repo `artifacts/hrc-dry-run-reports` folder remains absent after review and verification.

### Safety And Dashboard Review

The externally preserved artifact JSON set was reviewed with the read-only server API and the HRC Artifacts dashboard.

- privacy/path scan: PASS
- raw absolute path / user token / email / cloud-drive path leak: none found
- server API review: PASS
  - list items: 29
  - invalid items: 0
  - report/index/comparison kinds visible
- dashboard review: PASS
  - HRC Artifacts tab list/detail/safety badges reviewed
  - no import/export/run/upload/delete/write/solver/analyze buttons
- generated artifact JSON committed: no

### Import Candidate Classification

The v2.9 classification is advisory only. `IMPORT_CANDIDATE` means a zip is plausible for future v3.0 product import design review; it does not mean the zip is ready for DB insertion.
...
- dashboard reviewed count: 27

Important limits:

- No product import route connection.
- No `/api/imports/hrc` or `/api/imports/validate` artifact integration.
- No DB migration.
- No production DB write.
- No raw zip original commit.
- No raw zip full extraction commit.
- No generated artifact JSON commit.
- `amountUnit` remains `UNKNOWN`.
- `amountInterpretation` remains `RAW_HRC_AMOUNT_UNINTERPRETED`.
- v3.0 product import design must explicitly decide amount semantics, EV/strategy semantics, multi-node handling, and DB write policy before any import integration.
